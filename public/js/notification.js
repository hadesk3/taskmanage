...
function updateNotifications(alerts, unreadCount) {
    ...
    alerts.forEach((alertData) => {
        ...
        <div class="media align-items-center cust-card py-3 border-bottom special-alert">
          <div class="">
            <img class="avatar-50 rounded-small" src="${
                alertData.userImage || "/images/user/01.jpg"
            }" alt="user-img" />
          </div>
          <div class="media-body ml-3">
            <div class="d-flex align-items-center justify-content-between">
              <h6 class="mb-0 text-danger">Admin (Special)</h6>
              <small class="text-dark"><b>${new Date(
                  alertData.timestamp
              ).toLocaleTimeString()}</b></small>
            </div>
            <small class="mb-0">
              ...
              <strong>Extend Until:</strong> ${
                  alertData.date_extend
                      ? new Date(alertData.date_extend).toLocaleDateString()
                      : "N/A"
              } <br />
